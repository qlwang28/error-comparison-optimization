...
class DataProcessor(object):
    ...
    def _read_txt(cls, input_path):
        ...
        with open(input_path, 'r') as file:
            for line in file:
                labels_part = line.strip().split("####")[1]
                tokens, labels = [], []
                previous_label = None
                for token_label in labels_part.split():
                    ...
                    if label == 'O':
                        ...
                        if label != previous_label:
                            labels.append(label.replace("T-", "B-"))
                        else:
                            labels.append(label.replace("T-", "I-"))
                    
                    previous_label = label
                lines.append((" ".join(tokens), labels))
            return lines


class EntityProcessor(DataProcessor):
    def get_examples(self, data_dir, data_name):
        return self._create_examples(self._read_txt(os.path.join(data_dir, data_name+".txt")), data_name)
    def get_labels(self):
        return {"B-POS":0, "I-POS":1, "B-NEU":2, "I-NEU":3, "B-NEG":4, "I-NEG":5, "O":6}
        # return {"B-POS":0, "I-POS":1, "E-POS":2, "S-POS":3, "B-NEU":4, "I-NEU":5,  "E-NEU":6,  "S-NEU":7, "B-NEG":8, "I-NEG":9, "E-NEG":10, "S-NEG":11, "O":12}
